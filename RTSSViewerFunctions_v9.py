...
class ImageToneCorrection:
    ...
    def ToneCorrection_activate_event(self,event):
        if self.Function_Balance_Control.ImageToneCorrection_FLAG and event.button==3:
            ...
            hist_ax.set_xlim(self.ctvolume.vmin,self.ctvolume.vmax)
            hist_ax.set_xticks([self.ctvolume.vmin,self.ctvolume.vmax])
            hist_ax.set_yticks([])
            #現在の画素値範囲,元の画素値範囲
            ...
            self.upper_limit_line=hist_ax.axvline(current_upper,color="#0000FF",alpha=0.5)
            """
    ...
    def center_change_set(self,event):
        if self.change_target=='center':
            ...
            center=max(self.ctvolume.vmin+self.range,min(self.center+diff_x,self.ctvolume.vmax-self.range))
    ...
    def range_change_set(self,event):
        if self.change_target=='range':
            if event.button=='up':#範囲拡大
                change_value=self.range_step
            elif event.button=='down':#範囲縮小
                change_value=(-self.range_step)
            
            range=max(0,self.range+change_value)
            if self.range!=range:
                lower=max(self.ctvolume.vmin,self.center-range)
                upper=min(self.center+range,self.ctvolume.vmax)
                self.range=range
