...
class ROISelecter:
    ...
    def __init__(self,base,Function_Balance_Control):
        ...
        self.key_list=list(self.rtss.contours.keys())
        self.ROI_kinds=len(self.key_list)
        self.ROIName_maxlength=len(max(self.key_list,key=lambda x:len(x)))
        print(f"{self.__class__.__name__} Registerd !")
    ...
    def ROIselecter_activate_event(self,event):
        if self.Function_Balance_Control.ROISelecter_FLAG and event.button==3:
            ...
            columns_num=math.ceil(self.ROI_kinds/30)
            #columns_num=2
            #window用意
            self.ROISelectWindow_fig=plt.figure(f"{self.ROI_kinds} ROIs",figsize=(0.1*max_length*columns_num+0.5,self.ROI_kinds/(columns_num*5)))
            gs=gridspec.GridSpec(1,columns_num,hspace=0,wspace=0,top=1,bottom=0,right=1,left=0)
            #配置決定
            base_rows_num=self.ROI_kinds//columns_num
            surplus_num=self.ROI_kinds%columns_num
    ...
    def ROI_clicked(self,label):
        ...
        status=self.CheckButton_list[index[1]].get_status()[index[0]]
    ...
    def space_pressed_event(self,event):
        if event.key==" ":
            ...
            for cb in self.CheckButton_list:
                sub_status_list=cb.get_status()
                for i in sub_status_list:
                    RESET_FLAG*=i
                length_list.append(len(sub_status_list))

            for cb,length in zip(self.CheckButton_list,length_list):
                for i in range(length):
                    cb.set_active(i,(not RESET_FLAG))
    ...
    def close(self,event):
        ...
        with open(save_filepath,"w") as f:
            for checkbutton in self.CheckButton_list:
                for ROIName in checkbutton.get_checked_labels():
                    count+=1
                    text=f"{ROIName}, {", ".join([str(v) for v in self.rtss.get_Range(ROIName)])}"
                    print(text)
                    f.write(f"{text}\n")
        
        print(f"----------------------\n{count} 種のROIを {save_filepath} に保存")